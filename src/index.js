...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if (user.pro || (!user.pro && userTodos < 10)) {
    return next();
  }

  return response.status(403).json({ error: "User reached free todos limit" });
}
...
function checksTodoExists(request, response, next) {
  ...
  if (!validate(id))
    return response
      .status(400)
      .json({ error: "Todo does not have a valid id" });

  const foundTodo = foundUser.todos.find((todo) => todo.id === id);

  if (!foundTodo) return response.status(404).json({ error: "Id not found" });
